...
export async function createLicensesForDashboard({ count = 1, durationDays = 1, notes = '' } = {}) {
...
export async function completeOrderPayment(reference, providerReference = null, paidAmount = null, raw = {}) {
  const existing = await getOrderPaymentByReference(reference);
  if (!existing) {
  ...
  const updatePayload = {
    ...
    provider_reference: providerReference || existing.provider_reference || null,
    raw_response: raw || {}
  };

  if (paidAmount !== null && paidAmount !== undefined && !Number.isNaN(Number(paidAmount))) {
    updatePayload.paid_amount = Number(paidAmount);
  }

  return await updateOrderPaymentByReference(reference, updatePayload);
}

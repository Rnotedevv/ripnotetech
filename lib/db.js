...
export async function getAvailableInventoryByProductCode(productCode, limit = 100) {
  const code = String(productCode || '').trim();

  if (!code) {
    throw new Error('Kode product tidak valid.');
  }
  ...
  const { data, error } = await db()
    ...
    .or('is_sold.is.false,is_sold.is.null')
...
export async function findSoldEmailHistory(email) {
  ...
  const { data, error } = await db()
    ...
    .select(`
      *,
      buyer:users!inventory_items_sold_to_user_id_fkey (
        id,
        username,
        full_name,
        tg_user_id
...
export async function takeInventoryForManualSend(productCode, qty, targetUserId = null) {
  const code = String(productCode || '').trim();
  if (!code) {
    throw new Error('Kode product tidak valid.');
  }
  ...
  if (!data || data.length < safeQty) {
    throw new Error(`Stok tidak cukup. Tersedia hanya ${data?.length || 0}.`);
  }
  ...
  const updatePayload = {
    ...
    sold_order_code: 'MANUAL-SENDSTOCK'
  };

  if (targetUserId) {
    updatePayload.sold_to_user_id = targetUserId;
  }
...
export function extractDomainFromInventoryContent(content = '') {
  const raw = String(content || '').trim();

  if (!raw) return null;

  const firstPart = raw.split(':')[0].trim().toLowerCase();
  const atIndex = firstPart.lastIndexOf('@');

  if (atIndex === -1) return null;

  const domain = firstPart.slice(atIndex + 1).trim().toLowerCase();
  return domain || null;
}

export async function incrementSoldDomainStats(productCode, items = []) {
  const cleanProductCode = String(productCode || '').trim();
  if (!cleanProductCode || !Array.isArray(items) || !items.length) return;
  ...
  for (const item of items) {
    ...
    const domain = extractDomainFromInventoryContent(rawContent);
    if (!domain) continue;
  ...
  for (const [domain, count] of domainCountMap.entries()) {
    const { data: existing, error: findError } = await db()
      .from('sold_domain_stats')
      .select('*')
      .eq('product_code', cleanProductCode)
      .eq('domain', domain)
      .maybeSingle();
    ...
    if (existing) {
      const { error: updateError } = await db()
        .from('sold_domain_stats')
        .update({
          sold_count: Number(existing.sold_count || 0) + count,
...
export async function getSoldDomainStatsByProductCodes(productCodes = []) {
...
export async function clearSoldDomainStats(productCodes = []) {
  const codes = productCodes.map((code) => String(code).trim()).filter(Boolean);

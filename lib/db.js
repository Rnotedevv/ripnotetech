...
function generateLicenseKey(prefix = 'LIC') {
  const chunk = (size = 4) => randomBytes(size).toString('hex').slice(0, size).toUpperCase();
  return `${prefix}-${chunk(4)}-${chunk(4)}-${chunk(4)}-${chunk(4)}`;
}
...
export async function createLicensesForDashboard({ quantity = 1, durationDays = 1, notes = '' }) {
  const rows = Array.from({ length: quantity }, () => ({
    license_key: generateLicenseKey('AUTOFIL'),
    status: 'unused',
    duration_days: durationDays,
    notes: notes || null
  }));

  const { data, error } = await db().from('licenses').insert(rows).select('*');
  if (error) throw error;
  return data || [];
}

export async function revokeLicenseById(licenseId) {
  const { error } = await db()
    .from('licenses')
...
export async function reserveFakerEmail({ email, source = 'extension', licenseKey = null, siteHost = null, notes = null }) {
  ...
  const { data, error } = await db()
    ...
    .insert({
      ...
      source: String(source || 'extension').trim() || 'extension',
  ...
  if (error) {
    if (error.code === '23505') {
      throw new Error('Email ini sudah pernah dipakai.');
    }
    throw error;
  }

...
export async function answerCallbackQuery(callbackQueryId, text = '') {
  try {
    return await telegramCall('answerCallbackQuery', {
    ...
    if (
      ...
      console.warn('answerCallbackQuery skipped:', message);
      return null;
    }
    throw error;
  }
...
if (data.startsWith('check_deposit:')) {
  ...
  await answerCallbackQuery(callbackQuery.id, 'Mengecek status...');
  ...
  try {
    ...
    return sendMessage(chatId, 'Gagal cek status gateway.');
  }
...
export async function notifyAdminGroup(text, extra = {}) {
  ...
  try {
    return await sendMessage(env.adminGroupChatId, text, {
      parse_mode: 'HTML',

...
import {
  incrementSoldDomainStats,
getSoldDomainStatsByProductCodes,
clearSoldDomainStats,
  isUserApproved,
...
await sendMessage(
    2063218182,   // ← ID private admin kamu
    [
      '📦 <b>User cek stok</b>',
    ...
    { parse_mode: 'HTML' }
  ).catch(() => null);   // biar tidak error kalau admin block bot
...
async function finalizePurchase(chatId, tgUserId, productId, qty) {
  ...
  try {
    ...
    await incrementSoldDomainStats(product?.product_code, items).catch((error) => {
  console.error('incrementSoldDomainStats finalizePurchase error', error);
});
    const deliveryText = [
...
if (command === '/cek') {
  try {
    const rows = await getSoldDomainStatsByProductCodes(['1', '2', '3']);
    ...
    const formatGroup = (code) => {
      const items = grouped[code] || [];
      if (!items.length) {
        return [
          `<b>Product ${code}</b>`,
          'Belum ada domain terjual.'
        ].join('\n');
      }

      return [
        `<b>Product ${code}</b>`,
        ...items.map((item) => `- <code>${escapeHtml(item.domain)}</code> = <b>${item.sold_count}</b>`)
      ].join('\n');
    };
    ...
    await sendMessage(chatId, `Gagal cek monitor domain: ${error.message}`);
  }
...
if (command === '/clearcek') {
  try {
    await clearSoldDomainStats(['1', '2', '3']);

    await sendMessage(
      chatId,
      '✅ Monitor penjualan domain untuk product 1, 2, dan 3 berhasil direset.'
    );
  } catch (error) {
    await sendMessage(chatId, `Gagal reset monitor domain: ${error.message}`);
  }
...
if (text === settings.menu_deposit_label) {
  await clearUserState(user.id);
  await setUserState(user.id, STATE_WAITING_DEPOSIT, {}, 999);
  return sendMessage(
    chatId,
...
if (data.startsWith('buy_balance:')) {
 ...
 if (data.startsWith('buy_qris:')) {
  ...
  const soldProduct = await getProductById(orderPay.product_id).catch(() => null);
  const soldItems = Array.isArray(result?.items) ? result.items : [];

  await incrementSoldDomainStats(
    String(soldProduct?.product_code || orderPay.raw_response?.product?.product_code || ''),
    soldItems
  ).catch((error) => {
    console.error('incrementSoldDomainStats check_orderpay error', error);
  });
  ...
  if (!result?.already_paid) {
    ...
    const deliveryText = [
      ...
      ...(Array.isArray(result.items) ? result.items.map((item) => `${item}`) : [])
    ]
      .filter((v) => v !== null && v !== '')
      .join('\n');
...
if (command === '/removeuser') {
  ...
  try {
    ...
    const lines = result.items.map((item, index) => `${item.content}`);
...
// ←←← DEBUG DEPOSIT (biar kita tau kenapa state ga ke-detect)
console.log('=== DEBUG DEPOSIT STATE ===');
console.log('Text diterima     :', text);
console.log('State saat ini    :', state ? JSON.stringify(state, null, 2) : null);
console.log('Cocok WAITING_DEPOSIT?', state?.state_key === STATE_WAITING_DEPOSIT);
...
export async function processPaymentWebhookEvent(event) {
  ...
  if (event.status === 'paid') {
    if (reference.startsWith('ORDPAY-')) {
      ...
      const items = Array.isArray(result.items) ? result.items : [];
      await incrementSoldDomainStats(
  String(orderPay.raw_response?.product?.product_code || ''),
  items
).catch((error) => {
  console.error('incrementSoldDomainStats webhook error', error);
});
      const deliveryText = [
        result.delivery_note || '',
        result.delivery_note ? '' : null,

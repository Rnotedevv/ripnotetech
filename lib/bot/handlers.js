...
async function sendApprovalRequired(chatId) {
  return sendMessage(
    chatId,
    '/reqacc dulu, Silakan hubungi http://wa.me/62858171868131 untuk meminta acc. Setelah di-ACC admin, kamu baru bisa memakai bot ini.'
  );
}
    ...
    for (const row of batch) {
      ...
      await sendMessage(
        ...
        [
          ...
          `Stok sekarang: <b>${availableStock}</b>`,
          '',
...
if (data.startsWith('select_product:')) {
  ...
  if (!isAdminTelegramId(from.id) && !approved) {
    await answerCallbackQuery(callbackQuery.id, 'Harus ACC dulu');
    return sendApprovalRequired(chatId);
  }
  ...
  if (data.startsWith('upload_product:')) {
    ...
    if (!product) {
      await answerCallbackQuery(callbackQuery.id, 'Produk tidak ditemukan');
      return sendMessage(chatId, 'Produk tidak ditemukan.');
    }

    await clearUserState(user.id);
    await setUserState(user.id, STATE_WAITING_UPLOAD_LINES, { productId }, 30);

    await answerCallbackQuery(callbackQuery.id, 'Produk dipilih');

    return sendMessage(
      chatId,
      `Kirim stok untuk <b>${product.name}</b>\n\n1 baris = 1 item`,
      { parse_mode: 'HTML' }
    );
  }
...
if (!isAdmin && !approved && text === settings.menu_buy_label) {
  return sendApprovalRequired(chatId);
}
...
if (command === '/reqacc') {
    ...
    if (command === '/upload' && isAdmin) {
      ...
      return sendMessage(
        chatId,
        'Pilih produk yang ingin diupload stoknya:',
        {
          reply_markup: {
            inline_keyboard: products.map((product) => [
              {
                text: `${product.name} (${product.available_stock})`,
                callback_data: `upload_product:${product.id}`
              }
            ])
          }
        }

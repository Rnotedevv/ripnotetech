...
async function handleCallback(callbackQuery) {
  ...
  if (data.startsWith('buy_qris:')) {
    ...
    return startOrderQrisPayment(chatId, user, Number(productId), Number(qty));
  }
  ...
  if (data.startsWith('check_orderpay:')) {
    ...
    if (!orderPay) {
      await answerCallbackQuery(callbackQuery.id, 'Order payment tidak ditemukan');
      return sendMessage(chatId, 'Order payment tidak ditemukan.');
    }
    ...
    try {
      ...
      if (trxStatus === 'completed') {
        await processPaymentWebhookEvent({
          reference: orderPay.reference,
          providerReference: trx.order_id || orderPay.reference,
          amount: Number(trx.amount || orderPay.amount),
          status: 'paid',
          raw: { transaction_detail: trx, source: 'manual_check' }
        });

        orderPay = await getOrderPaymentByReference(reference);
        await answerCallbackQuery(callbackQuery.id, 'Pembayaran order sudah masuk');
      } else {
        await updateOrderPaymentByReference(reference, {
          status: ['expired', 'failed', 'cancelled'].includes(trxStatus) ? trxStatus : 'pending',
          raw_response: {
        ...
        await answerCallbackQuery(
          callbackQuery.id,
          `Status order: ${orderPay?.status || trxStatus || 'pending'}`
        );
      }
    } catch (error) {
      console.error('Check orderpay error:', error);
      await answerCallbackQuery(callbackQuery.id, 'Gagal cek status');
    }

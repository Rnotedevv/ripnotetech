...
export async function processTelegramUpdate(update) {
  ...
  if (text.startsWith('/')) {
    ...
    if (adminHandled?.handled) return null;
  }
...
if (state?.state_key === STATE_WAITING_UPLOAD_PRODUCT && isAdmin) {
  ...
  if (!uuidRegex.test(productId)) {
    return sendMessage(
      chatId,
      'Product ID tidak valid. Kirim UUID product yang benar, atau ketik /reset untuk batal.'
    );
  }
  ...
  if (!product) {
    return sendMessage(
      chatId,
      'Product tidak ditemukan. Masukkan Product ID yang valid, atau ketik /reset untuk batal.'
    );
  }
  ...
  if (state?.state_key === STATE_WAITING_UPLOAD_LINES && isAdmin) {
    ...
    if (!lines.length) {
      return sendMessage(chatId, 'Data kosong.');
    }

    const count = await bulkInsertInventory(state.payload.productId, lines);
    await clearUserState(user.id);

    return sendMessage(
      chatId,
      `✅ Upload berhasil: ${count} item`,
      { reply_markup: mainMenuKeyboard(settings, isAdmin) }
    );
  }

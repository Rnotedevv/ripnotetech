...
if (data.startsWith('buy_balance:')) {
 ...
 if (data.startsWith('upload_product:')) {
  const uploadAllowed = await canUseUpload(from, callbackQuery.message?.chat);
...
if (command === '/upload' && String(args[0] || '').trim() === '3') {
  ...
  const product = await getProductByCode('3');
  if (!product) {
    return sendMessage(chatId, 'Produk dengan kode 3 tidak ditemukan.');
  }

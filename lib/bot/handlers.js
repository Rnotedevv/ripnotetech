...
if (data.startsWith('buy_qris:')) {
  ...
  if (pendingOrder) {
    await answerCallbackQuery(callbackQuery.id, 'Masih ada pembayaran pending');

    return sendMessage(
      chatId,
      [
        '<b>Masih ada pembayaran pending untuk produk ini</b>',
        `Ref: <code>${escapeHtml(pendingOrder.reference)}</code>`,
        `Nominal: <b>${formatRupiah(pendingOrder.amount)}</b>`,
        'Selesaikan pembayaran lama atau tunggu sampai expired sebelum membuat invoice baru.'
      ].join('\n'),
      { parse_mode: 'HTML' }
    );
  }

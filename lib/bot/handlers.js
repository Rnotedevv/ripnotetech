...
async function handleUserText(message, user, settings, isAdmin) {
  ...
  // 1) COMMAND SELALU MENANG
  if (text === '/start' || text === '/menu') {
    await clearUserState(user.id);
    return sendMainMenu(chatId, settings, isAdmin);
  }

  // 2) MENU UTAMA SELALU MENANG DAN RESET STATE LAMA
  ...
  if (text === settings.menu_warranty_label) {
    ...
    return sendMessage(
      chatId,
      'Paste email yang bermasalah untuk diteruskan ke admin.',
      { reply_markup: mainMenuKeyboard(settings, isAdmin) }
    );
  }
  ...
  // opsional: command reset manual
  if (text === '/reset' || text === 'reset') {
    return resetAndMainMenu();
  }

  // 3) BARU SETELAH MENU DIPROSES, CEK STATE
  ...
  // 4) FALLBACK: KALAU TEXT TIDAK DIKENAL, RESET SAJA
  await clearUserState(user.id);
...
if (data.startsWith('check_deposit:')) {
  ...
  try {
    ...
    if (trxStatus === 'completed') {
      ...
      deposit = await getDepositByReference(reference);
      await answerCallbackQuery(callbackQuery.id, `Status deposit: ${deposit?.status || trxStatus || 'pending'}`);
    }
  } catch (error) {
